...
export default {
  ...
  mutations: {
    ...
    CALCULATE_DIIFS(state) {
      if (state.series.length) {
        ...
        state.diffPercent =
          100 - new Calc(start).multiply(100).divide(end).finish();
      }
    },
  ...
  actions: {
    fetchTimeseries(
      { commit },
      { currency, interval, period, start_date, end_date }
    ) {
      let query = {
        currency,
        interval,
      ...
      return get("/timeseries", query).then(({ quotes }) => {
        quotes = quotes.map(({ close, date }) => {
    ...
    connectSocket({ getters, commit }) {
      const serverUrl = import.meta.env.VITE_SOCKET_BASE_API_URL;

      const socket_key = import.meta.env.VITE_SOCKET_API_KEY;
